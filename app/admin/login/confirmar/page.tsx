...
export default function ConfirmarDono({ searchParams }: { searchParams: Promise<{ telefone?: string; nome?: string }> }) {
  ...
  async function confirmar() {
    ...
    if (!response.ok || !result.session) { setLoading(false); return setErro(result.error ?? "Código inválido."); }
    const { error } = await createClient().auth.setSession(result.session);
    if (error) { setLoading(false); return setErro(error.message); }
    const profile = await fetch("/api/admin/registrar", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nome }),
    });
    if (!profile.ok) { setLoading(false); return setErro((await profile.json()).error ?? "Não foi possível criar o acesso."); }
    router.push("/admin/carteira");
  }

  return <main className="flex flex-1 flex-col justify-center px-6"><h1 className="text-center text-lg font-medium">Confirme seu código</h1><p className="mt-2 text-center text-sm text-ink-soft">Enviamos um código para {telefone}</p><input value={codigo} onChange={e => setCodigo(e.target.value)} maxLength={6} inputMode="numeric" placeholder="000000" className="mt-6 rounded-xl border border-border-strong px-3 py-4 text-center text-lg tracking-[.4em] outline-none" />{erro && <p className="mt-3 text-center text-xs text-red-dark">{erro}</p>}<div className="mt-5"><Button onClick={confirmar} disabled={loading || codigo.length < 4}>{loading ? "Confirmando..." : "Confirmar acesso"}</Button></div></main>;
}
